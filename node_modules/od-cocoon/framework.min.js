...
namespace`w3c.ui`(class WebComponent extends HTMLElement{constructor(el){super();this.element=el;this.__proto=this.constructor.prototype;this.root=this.onEnableShadow()?this.attachShadow({mode:"open"}):this.element||this;if(this.element)this.decorate()}static define(proto,bool){var tag=proto.classname.replace(/([a-zA-Z])(?=[A-Z0-9])/g,(f,m)=>`${m}-`).toLowerCase();if(/\-/.test(tag)){if(window.customElements.get(tag))return;console.info("Defining tag",tag);proto["ns-tagname"]=tag;this.defineAncestors();
this.defineAncestralClassList();try{window.customElements&&window.customElements.define(tag,this)}catch(e){console.error(e)}}else console.warn(`${proto.namespace}#define() - invalid tag name. Dashes required for`,tag)}setStylesheet(){var css=this.cssStyle();!!css&&!this.__proto._style_defined?(this.onAppendStyle(`<style>\n${css}\n</style>`.toDomElement()),this.__proto._style_defined=true):null}querySelector(cssSel,e){if(e)return this.getParentNodeFromEvent(e,cssSel);else return this.onEnableShadow()||
...
handler,bool=false,el){this.addEventListener(evtName,handler,bool,el)}addEventListener(evtName,handler,bool=false,el){var self=this;if(typeof el=="string")this.addEventListener(evtName,(e)=>{var t=this.getParentNodeFromEvent(e,el);if(t)handler({target:t,realtarget:e.target,src:e,preventDefault:()=>e.preventDefault(),stopPropagation:()=>e.stopPropagation()})},bool);else super.addEventListener(evtName,handler,bool)}getParentBySelectorUntil(elem=this,terminator="html",selector){var parent_node=null;
do{if(elem.matches(selector)){parent_node=elem;break}if(elem.matches(terminator))break;elem=elem.parentNode}while(elem&&elem.matches);return parent_node}getParentNodeFromEvent(e,selector,terminator){var el=e.composedPath()[0];return this.getParentBySelectorUntil(el,terminator,selector)}onStylesheetLoaded(style){}onTransformStyle(css){return css}setCssTextAttribute(_cssText,stylenode){if(stylenode&&stylenode.styleSheet)stylenode.styleSheet.cssText=_cssText;else stylenode.appendChild(document.createTextNode(_cssText))}async loadTemplate(){return new Promise(async(resolve,
reject)=>{var tem=this.getTemplateToLoad();if(/\/*\.html$/.test(tem)){var src=this.src||tem;var opts={cache:"force-cache"};src=src.replace("/./","/"+this.namespace.replace(/\./gim,"/")+"/");this._template=await imports(src,opts)}else if(/<\s*\btemplate\b/.test(tem))this._template=tem;else if(tem&&tem.nodeType==1)this._template=tem.outerHTML;resolve(this._template)})}getTemplateToLoad(){var engine=this.getTemplateEngine();return this.querySelector("template")||this.src||this.template()||"/src/./index"+
(engine.ext||"")+".html"}async onConnected(data){await this.render(data)}async render(data){if(this.element)return;data=data||{};var t=this._template;if(t){var html=await this.evalTemplate(t,data||{});var temNode=html.toDomElement();temNode=temNode.content;if(!this.onEnableShadow())this.slots.forEach((slot)=>{var slotName=slot.getAttribute("slot");var placeholder=temNode.querySelector(`slot[name="${slotName}"]`);if(placeholder)if(!placeholder.hasAttribute("append"))placeholder.innerHTML="";(placeholder||
temNode).appendChild(slot)});this.root.innerHTML="";this.root.appendChild(temNode);this.onTemplateRendered(temNode)}}template(){return null}async evalTemplate(template,data){var eng=this.getTemplateEngine();return await eng.parse(template,data,this)}getTemplateEngine(){return window.customTemplateEngines.default}async connectedCallback(){if(this._is_connected)return;this._is_connected=true;var html=await this.loadTemplate();this.onTemplateLoaded()}decorate(){this.setClassList();this.setPrototypeInstance();
this.defineAncestralStyleList();this.setStyleDocuments();this.onConnected()}async onTemplateLoaded(){this.slots=this.getSlots();this.setClassList();this.setPrototypeInstance();this.defineAncestralStyleList();await this.onConnected();await this.setStyleDocuments()}getSlots(){return Array.from(this.children)}onTemplateRendered(){this.initializeChildComponents()}static get observedAttributes(){return["src"]}get src(){return this.getAttribute("src")}set src(val){this.setAttribute("src",val)}onEnableShadow(){return this.hasAttribute("shadow")}attachShadow(options){this._usesShadow=
...
ancestor==this)this.prototype.classes.unshift(proto.classname);else break}}defineAncestralStyleList(){var stylesheets=this.prototype["stylesheets"]=this.prototype["stylesheets"]||[];if(this.onLoadInstanceStylesheet())stylesheets.push(this.getNSStyleSheet(this.namespace));stylesheets.push(...this.prototype["@stylesheets"]||[]);if(!this["@cascade"])return;var ancestor=this.constructor.prototype.ancestor;while(ancestor)if(ancestor!=w3c.ui.WebComponent&&ancestor!=w3c.ui.Application){stylesheets.unshift(...ancestor.prototype["@stylesheets"]||
[]);stylesheets.unshift(this.getNSStyleSheet(ancestor.prototype.namespace));ancestor=ancestor.prototype.ancestor}else break}getNSStyleSheet(ns){return relativeToAbsoluteFilePath("/src/./index.css",ns)}setClassList(){this.root.className=this.root.className+(this["@cascade"]?" "+this.constructor.prototype.classes.join(" ").trim():" "+this.classname)}getStyleSheets(){var styles=this["stylesheets"]||[];if(styles.length<=0&&this.onLoadInstanceStylesheet())styles.push(this.getNSStyleSheet(this.namespace));
return styles.reverse()}async setStyleDocuments(){await this.loadcss(this.getStyleSheets());this.setStylesheet();this.onStyleComputed(this.stylesheets)}async loadcss(urls){return new Promise(async(resolve,reject)=>{if(this.constructor.prototype._css_loaded){resolve(true);return}else console.log(this.namespace+" loading",urls);this.constructor.prototype._css_loaded=true;urls=urls.reverse();var stylesheets=window.loaded_stylesheets=window.loaded_stylesheets||{};for(let path of urls){path=this.onLoadStyle(path);
if(path&&!stylesheets[path]){var tagName=/^http/.test(path)?"link":"style";var tag=document.createElement(tagName);this.onAppendStyle(tag);tag.setAttribute("type","text/css");tag.setAttribute("rel","stylesheet");tag.setAttribute("href",path);tag.setAttribute("component",this.namespace);stylesheets[path]=tag;if(tagName.toLowerCase()=="style"){var _cssText=await window.imports(path);if(_cssText){_cssText=this.onTransformStyle(_cssText);_cssText&&this.setCssTextAttribute(_cssText,tag);this.onStylesheetLoaded(tag)}}}}resolve(true)})}onLoadStyle(url){return url}setPrototypeInstance(){this.root.setAttribute("namespace",
this.namespace);this.prototype=this}resourcepath(url,ns){url=url.replace(/\$\{ns\}/gm,ns.replace(/\./gim,"/"));return Config.ROOTPATH+url}initializeChildComponents(el){el=el||this;var self=this;var nodes=this.querySelectorAll("*");nodes=[].slice.call(nodes);nodes.forEach((n)=>{if(n&&n.nodeType==1){var tag=n.tagName.toLowerCase();var c=window.registered_tags[tag];c&&c.define(c.prototype,true)}})}isAnyPartOfElementInViewport(el=this){var rect=el.getBoundingClientRect();var v=rect.top<=window.innerHeight&&
rect.bottom>=0;var h=rect.left<=window.innerWidth&&rect.right>=0;return v&&h}});cascade(w3c.ui.WebComponent,true);
namespace`w3c.ui`(class Application extends w3c.ui.WebComponent{constructor(el){super(el);window.application=this;window.sprites=[];this.head=document.getElementsByTagName("head")[0];this.configscript=document.querySelector("script[id='config']")||document.querySelector("script")}onUpdate(time){window.sprites.forEach((sprite)=>sprite.onUpdate(time))}onDraw(interpolation){if(this.context)this.context.clearRect(0,0,this.canvas.width,this.canvas.height);window.sprites.forEach((sprite)=>sprite.onDraw(interpolation,
this.context))}onEnd(fps,panic){if(panic){var discardedTime=Math.round(MainLoop.resetFrameDelta());console.warn("Main loop panicked, probably because the browser tab was put in the background. Discarding "+discardedTime+"ms")}}static define(proto,bool=true){super.define(proto,bool)}async onConnected(){await this.render()}});function Ecmascript6ClassTranspiler(){}Ecmascript6ClassTranspiler.prototype.imports=window.imports;
