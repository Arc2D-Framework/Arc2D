...
[];var a=this;while(a&&this.ancestors.push(a))a=a.prototype.ancestor}static defineAncestralClassList(){this.prototype.classes=[];for(let ancestor of this.ancestors){var proto=ancestor.prototype;if(proto["@cascade"]||ancestor==this)this.prototype.classes.unshift(proto.classname);else break}}setClassList(){this.root.className=this.root.className+" "+this.constructor.prototype.classes.join(" ").trim()}getStyleSheets(){var styles=this["@stylesheets"]||[];return styles.reverse()}async loadcss(urls){urls=
...
this.onTransformStyle(_cssText);_cssText&&this.setCssTextAttribute(_cssText,tag);this.onStylesheetLoaded(tag);this.onAppendStyle(tag)}}else this.onAppendStyle(tag)}}}onLoadStyle(url){return url}setPrototypeInstance(){this.root.setAttribute("namespace",this.namespace);this.prototype=this}resourcepath(url,ns){url=url.replace(/\$\{ns\}/gm,ns.replace(/\./gim,"/"));return Config.ROOTPATH+url}initializeChildComponents(el){el=el||this;var self=this;var nodes=this.querySelectorAll("*");nodes=[].slice.call(nodes);
nodes.forEach((n)=>{if(n&&n.nodeType==1){var tag=n.tagName.toLowerCase();var c=window.registered_tags[tag];c&&c.define(c.prototype,true)}})}isAnyPartOfElementInViewport(el=this){var rect=el.getBoundingClientRect();var v=rect.top<=window.innerHeight&&rect.bottom>=0;var h=rect.left<=window.innerWidth&&rect.right>=0;return v&&h}});cascade(w3c.ui.WebComponent,true);
