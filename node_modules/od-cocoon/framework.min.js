...
window.loaded_stylesheets||{};for(let path of urls){path=this.onLoadStyle(path);if(path&&!stylesheets[path]){var tagName=/^http/.test(path)?"link":"style";var tag=document.createElement(tagName);tag.setAttribute("type","text/css");tag.setAttribute("rel","stylesheet");tag.setAttribute("href",path);tag.setAttribute("component",this.namespace);stylesheets[path]=tag;if(tagName.toLowerCase()=="style"){var _cssText=await window.imports(path);_cssText=this.cssTransform(_cssText);this.setCssTextAttribute(_cssText,
tag);this.onStylesheetLoaded(tag)}this.appendStyleSheet(tag)}}}onLoadStyle(url){return url}template(){return null}evalTemplate(template,data){var eng=this.getTemplateEngine();return eng.eval(template,data,this)}getTemplateEngine(){return window.customTemplateEngines.default}setPrototypeInstance(){this.setAttribute("namespace",this.namespace);this.prototype=this}resourcepath(url,ns){url=url.replace(/\$\{ns\}/gm,ns.replace(/\./gim,"/"));return Config.ROOTPATH+url}initializeChildComponents(el){el=el||
this;var self=this;var nodes=this.querySelectorAll("*");nodes=[].slice.call(nodes);nodes.forEach((n)=>{if(n&&n.nodeType==1){var tag=n.tagName.toLowerCase();var c=window.registered_tags[tag];c&&c.define(c.prototype,true)}})}isAnyPartOfElementInViewport(el=this){var rect=el.getBoundingClientRect();var v=rect.top<=window.innerHeight&&rect.bottom>=0;var h=rect.left<=window.innerWidth&&rect.right>=0;return v&&h}});cascade(w3c.ui.WebComponent,true);(()=>{})();
...
"cordova"in window?document.addEventListener("deviceready",()=>{AndroidFullScreen&&AndroidFullScreen.immersiveMode((e)=>{},(e)=>{});bootup()},false):bootup()},false);console.log(`\ud83e\udd8f Oros Engine - v${$framework.current}`);
