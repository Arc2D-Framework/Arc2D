...
(()=>{var ThreadedTemplateLiterals={name:"ThreadedTemplateLiterals",ext:"",parse:async function(tempStr,data,self){return new Promise(async(resolve,reject)=>{console.log("data",data);var thread=new core.lang.Thread(async function(e){var parse=new Function("return `"+e.data.template+"`;");postMessage(parse.call(e.data.dataobj))});thread.onmessage=function(event){console.log("worker",event.data);resolve(event.data)};thread.postMessage({template:tempStr,dataobj:data})})},install:function(){}};window.customTemplateEngines.define("template/threadedliterals",
...
this.src||tem;var opts={cache:"force-cache"};src=src.replace("/./","/"+this.namespace.replace(/\./gim,"/")+"/");this._template=await imports(src,opts)}else if(/<\s*\btemplate\b/.test(tem))this._template=tem;else if(tem&&tem.nodeType==1)this._template=tem.outerHTML;resolve(this._template)})}getTemplateToLoad(){var engine=this.getTemplateEngine();return this.querySelector("template")||this.src||this.template()||"/src/./index"+(engine.ext||"")+".html"}async onConnected(data){await this.render(data)}onPreConnected(){this.onConnected()}async render(data){if(this.element)return;
data=data||{};var t=this._template;if(t){var html=await this.evalTemplate(t,data||{});var temNode=html.toDomElement();temNode=temNode.content;if(!this.onEnableShadow())this.slots.forEach((slot)=>{var slotName=slot.getAttribute("slot");var placeholder=temNode.querySelector(`slot[name="${slotName}"]`);if(placeholder)if(!placeholder.hasAttribute("append"))placeholder.innerHTML="";(placeholder||temNode).appendChild(slot)});this.root.innerHTML="";this.root.appendChild(temNode);this.onTemplateRendered(temNode)}}template(){return null}async evalTemplate(template,
data){var eng=this.getTemplateEngine();return await eng.parse(template,data,this)}getTemplateEngine(){return window.customTemplateEngines.default}async connectedCallback(){if(this._is_connected)return;this._is_connected=true;var html=await this.loadTemplate();this.onTemplateLoaded()}decorate(){this.setClassList();this.setPrototypeInstance();this.setStyleDocuments();this.onPreConnected()}onTemplateLoaded(){this.slots=this.getSlots();this.setClassList();this.setPrototypeInstance();this.setStyleDocuments();
this.onPreConnected()}getSlots(){return Array.from(this.children)}onTemplateRendered(){this.initializeChildComponents()}static get observedAttributes(){return["src"]}get src(){return this.getAttribute("src")}set src(val){this.setAttribute("src",val)}onEnableShadow(){return this.hasAttribute("shadow")}attachShadow(options){this._usesShadow=true;return super.attachShadow(options)}async attributeChangedCallback(name,oldValue,newValue){if(name=="src")if(!this._is_connected)return;else{var html=await this.loadTemplate();
this.onPreConnected()}}cssStyle(){return""}setStyleDocuments(){this.loadcss(this.getStyleSheets());if(this.onLoadInstanceStylesheet())this.loadcss([this.getDefaultStyleSheet()]);this.setStylesheet()}onLoadInstanceStylesheet(){return true}static defineAncestors(){this.ancestors=[];var a=this;while(a&&this.ancestors.push(a))a=a.prototype.ancestor}static defineAncestralClassList(){this.prototype.classes=[];for(let ancestor of this.ancestors){var proto=ancestor.prototype;if(proto["@cascade"]||ancestor==
this)this.prototype.classes.unshift(proto.classname);else break}}setClassList(){this.root.className+=this.className+(" "+this.constructor.prototype.classes.join(" ")).trim()}getStyleSheets(){var styles=this["@stylesheets"]||[];return styles.reverse()}async loadcss(urls){urls=urls.reverse();var stylesheets=window.loaded_stylesheets=window.loaded_stylesheets||{};for(let path of urls){path=this.onLoadStyle(path);if(path&&!stylesheets[path]){var tagName=/^http/.test(path)?"link":"style";var tag=document.createElement(tagName);
