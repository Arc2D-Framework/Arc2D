...
window.customElements.define(tag,this)}catch(e){console.error(e)}}}setStylesheet(){var css=this.cssStyle();!!css&&!this.__proto._style_defined?(this.onAppendStyle(`<style type="text/css" rel="stylsheet">\n${css}\n</style>`.toDomElement()),this.__proto._style_defined=true):null}querySelector(cssSel,e){if(e)return this.getParentNodeFromEvent(e,cssSel);else return this.onEnableShadow()?this.root.querySelector(cssSel):super.querySelector(cssSel)}querySelectorAll(cssSel,deep){return this.onEnableShadow()?
this.root.querySelectorAll(cssSel):super.querySelectorAll(cssSel)}onAppendStyle(stylesheet){if(this.onEnableShadow())try{var style=new CSSStyleSheet;style.replace(stylesheet.innerText);this.root.adoptedStyleSheets=[stylesheet]}catch(e){console.error(`${e.message} Unable to adopt stylesheet \n                        into shadow dom -- ${this.namespace}#onAppendStyle(), \n                        see: https://bugzilla.mozilla.org/show_bug.cgi?id=1520690.\n                        As a workaround, @import the css from within <template>`)}else{var headNode=
...
selector,terminator){var el=e.composedPath()[0];return this.getParentBySelectorUntil(el,terminator,selector)}onStylesheetLoaded(style){}onTransformStyle(css){return css}setCssTextAttribute(_cssText,stylenode){if(stylenode&&stylenode.styleSheet)stylenode.styleSheet.cssText=_cssText;else stylenode.appendChild(document.createTextNode(_cssText))}async loadTemplate(){return new Promise(async(resolve,reject)=>{var tem=this.getTemplateToLoad();if(/\/*\.html$/.test(tem)){var src=this.src||tem;var opts={cache:"force-cache"};
