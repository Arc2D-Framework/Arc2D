...
[];for(let ancestor of this.ancestors){var proto=ancestor.prototype;if(proto["@cascade"]||ancestor==this)this.prototype.classes.unshift(proto.classname);else break}}static defineAncestralStyleList(){var stylesheets=this.prototype["stylesheets"]=[];for(let ancestor of this.ancestors)if(ancestor.prototype["@cascade"]&&ancestor!=w3c.ui.WebComponent&&ancestor!=w3c.ui.Application)stylesheets.unshift(this.getNSStyleSheet(ancestor.prototype.namespace));else break;wait(20).then((_)=>console.log(this.prototype.namespace,
stylesheets))}static getNSStyleSheet(ns){return relativeToAbsoluteFilePath("/src/./index.css",ns)}setClassList(){this.root.className=this.root.className+" "+this.constructor.prototype.classes.join(" ").trim()}getStyleSheets(){var styles=this["stylesheets"]||[];return styles.reverse()}setStyleDocuments(){this.loadcss(this.getStyleSheets());this.setStylesheet()}async loadcss(urls){urls=urls.reverse();var stylesheets=window.loaded_stylesheets=window.loaded_stylesheets||{};for(let path of urls){path=
this.onLoadStyle(path);if(path&&!stylesheets[path]){var tagName=/^http/.test(path)?"link":"style";var tag=document.createElement(tagName);tag.setAttribute("type","text/css");tag.setAttribute("rel","stylesheet");tag.setAttribute("href",path);tag.setAttribute("component",this.namespace);stylesheets[path]=tag;if(tagName.toLowerCase()=="style"){var _cssText=await window.imports(path);if(_cssText){_cssText=this.onTransformStyle(_cssText);_cssText&&this.setCssTextAttribute(_cssText,tag);this.onStylesheetLoaded(tag);
this.onAppendStyle(tag)}}else this.onAppendStyle(tag)}}}onLoadStyle(url){return url}setPrototypeInstance(){this.root.setAttribute("namespace",this.namespace);this.prototype=this}resourcepath(url,ns){url=url.replace(/\$\{ns\}/gm,ns.replace(/\./gim,"/"));return Config.ROOTPATH+url}initializeChildComponents(el){el=el||this;var self=this;var nodes=this.querySelectorAll("*");nodes=[].slice.call(nodes);nodes.forEach((n)=>{if(n&&n.nodeType==1){var tag=n.tagName.toLowerCase();var c=window.registered_tags[tag];
c&&c.define(c.prototype,true)}})}isAnyPartOfElementInViewport(el=this){var rect=el.getBoundingClientRect();var v=rect.top<=window.innerHeight&&rect.bottom>=0;var h=rect.left<=window.innerWidth&&rect.right>=0;return v&&h}});cascade(w3c.ui.WebComponent,true);
