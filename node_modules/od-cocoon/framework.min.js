...
false,el){this.addEventListener(evtName,handler,bool,el)}bind(el,evtName,handler,bool=false){console.warn(this.namespace+"#bind() - is deprecated",this);this.on(evtName,handler,bool,el)}addEventListener(evtName,handler,bool=false,el){var self=this;if(typeof el=="string")this.addEventListener(evtName,(e)=>{var t=this.getParentNodeFromEvent(e,el);if(t)handler({target:t,realtarget:e.target,src:e,preventDefault:()=>e.preventDefault,stopPropagation:()=>e.stopPropagation})},bool);else super.addEventListener(evtName,
handler,bool)}getParentBySelectorUntil(elem=this,terminator="html",selector){var parent_node=null;do{if(elem.matches(selector)){parent_node=elem;break}if(elem.matches(terminator))break;elem=elem.parentNode}while(elem&&elem.matches);return parent_node}getParentNodeFromEvent(e,selector,terminator){var el=e.composedPath()[0];return this.getParentBySelectorUntil(el,terminator,selector)}onStylesheetLoaded(style){}onTransformStyle(css){return css}setCssTextAttribute(_cssText,stylenode){if(stylenode&&stylenode.styleSheet)stylenode.styleSheet.cssText=
...
this.getTemplateEngine();return this.querySelector("template")||this.src||this.template()||"/src/./index"+(engine.ext||"")+".html"}async onConnected(data){await this.render(data)}onPreConnected(){this.onConnected()}async render(data){if(this.element)return;data=data||{};var t=this._template;if(t){var html=await this.evalTemplate(t,data||{});var temNode=html.toDomElement();temNode=temNode.content;if(!this.onEnableShadow())this.slots.forEach((slot)=>{var slotName=slot.getAttribute("slot");var placeholder=
temNode.querySelector(`slot[name="${slotName}"]`);if(placeholder)if(!placeholder.hasAttribute("append"))placeholder.innerHTML="";(placeholder||temNode).appendChild(slot)});this.root.innerHTML="";this.root.appendChild(temNode);this.onTemplateRendered(temNode)}}template(){return null}async evalTemplate(template,data){var eng=this.getTemplateEngine();return await eng.parse(template,data,this)}getTemplateEngine(){return window.customTemplateEngines.default}async connectedCallback(){if(this._is_connected)return;
this._is_connected=true;var html=await this.loadTemplate();this.onTemplateLoaded()}decorate(){this.setClassList();this.setPrototypeInstance();this.defineAncestralStyleList();this.setStyleDocuments();this.onPreConnected()}async onTemplateLoaded(){this.slots=this.getSlots();this.setClassList();this.setPrototypeInstance();this.defineAncestralStyleList();await this.onPreConnected();await this.setStyleDocuments()}getSlots(){return Array.from(this.children)}onTemplateRendered(){this.initializeChildComponents()}static get observedAttributes(){return["src"]}get src(){return this.getAttribute("src")}set src(val){this.setAttribute("src",
val)}onEnableShadow(){return this.hasAttribute("shadow")}attachShadow(options){this._usesShadow=true;return super.attachShadow(options)}async attributeChangedCallback(name,oldValue,newValue){if(name=="src")if(!this._is_connected)return;else{var html=await this.loadTemplate();this.onPreConnected()}}cssStyle(){return""}onLoadInstanceStylesheet(){return true}static defineAncestors(){this.ancestors=[];var a=this;while(a&&this.ancestors.push(a))a=a.prototype.ancestor}static defineAncestralClassList(){this.prototype.classes=
[];for(let ancestor of this.ancestors){var proto=ancestor.prototype;if(proto["@cascade"]||ancestor==this)this.prototype.classes.unshift(proto.classname);else break}}defineAncestralStyleList(){var stylesheets=this.prototype["stylesheets"]=this.prototype["stylesheets"]||[];stylesheets.push(this.getNSStyleSheet(this.namespace));stylesheets.push(...this.prototype["@stylesheets"]||[]);if(!this["@cascade"])return;var ancestor=this.constructor.prototype.ancestor;while(ancestor)if(ancestor!=w3c.ui.WebComponent&&
ancestor!=w3c.ui.Application){stylesheets.unshift(...ancestor.prototype["@stylesheets"]||[]);stylesheets.unshift(this.getNSStyleSheet(ancestor.prototype.namespace));ancestor=ancestor.prototype.ancestor}else break}getNSStyleSheet(ns){return relativeToAbsoluteFilePath("/src/./index.css",ns)}setClassList(){this.root.className=this.root.className+(this["@cascade"]?" "+this.constructor.prototype.classes.join(" ").trim():" "+this.classname)}getStyleSheets(){var styles=this["stylesheets"]||[];if(styles.length<=
0)styles.push(this.getNSStyleSheet(this.namespace));return styles.reverse()}async setStyleDocuments(){await this.loadcss(this.getStyleSheets());this.setStylesheet();this.onStyleComputed(this.stylesheets)}async loadcss(urls){return new Promise(async(resolve,reject)=>{if(this.constructor.prototype._css_loaded){resolve(true);return}else console.log(this.namespace+" loading",urls);this.constructor.prototype._css_loaded=true;urls=urls.reverse();var stylesheets=window.loaded_stylesheets=window.loaded_stylesheets||
{};for(let path of urls){path=this.onLoadStyle(path);if(path&&!stylesheets[path]){var tagName=/^http/.test(path)?"link":"style";var tag=document.createElement(tagName);this.onAppendStyle(tag);tag.setAttribute("type","text/css");tag.setAttribute("rel","stylesheet");tag.setAttribute("href",path);tag.setAttribute("component",this.namespace);stylesheets[path]=tag;if(tagName.toLowerCase()=="style"){var _cssText=await window.imports(path);if(_cssText){_cssText=this.onTransformStyle(_cssText);_cssText&&
this.setCssTextAttribute(_cssText,tag);this.onStylesheetLoaded(tag)}}}}resolve(true)})}onLoadStyle(url){return url}setPrototypeInstance(){this.root.setAttribute("namespace",this.namespace);this.prototype=this}resourcepath(url,ns){url=url.replace(/\$\{ns\}/gm,ns.replace(/\./gim,"/"));return Config.ROOTPATH+url}initializeChildComponents(el){el=el||this;var self=this;var nodes=this.querySelectorAll("*");nodes=[].slice.call(nodes);nodes.forEach((n)=>{if(n&&n.nodeType==1){var tag=n.tagName.toLowerCase();
