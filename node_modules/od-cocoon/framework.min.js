...
function relativeToAbsoluteFilePath(path,ns,appendRoot){ns=ns||this.namespace;ns=ns.replace(/\./gim,"/");if(path.indexOf("/./")>=0)path=path.replace("./",ns+"/");path=/http:/.test(path)?path:path.replace("//","/");return path}window.stylesheets=function stylesheets(target,paths){paths&&paths.forEach((p)=>{var filepath=relativeToAbsoluteFilePath(p,target.prototype.namespace,false);target.prototype["stylesheets"].push(filepath)})};
...
[];for(let ancestor of this.ancestors){var proto=ancestor.prototype;if(proto["@cascade"]||ancestor==this)this.prototype.classes.unshift(proto.classname);else break}}static defineAncestralStyleList(){var stylesheets=this.prototype["stylesheets"]=[];stylesheets.push(this.getNSStyleSheet(this.prototype.namespace));var ancestor=this.prototype.ancestor;while(ancestor)if(ancestor.prototype["@cascade"]&&ancestor!=w3c.ui.WebComponent&&ancestor!=w3c.ui.Application){stylesheets.unshift(this.getNSStyleSheet(ancestor.prototype.namespace));
ancestor=ancestor.prototype.ancestor}else break}static getNSStyleSheet(ns){return relativeToAbsoluteFilePath("/src/./index.css",ns)}setClassList(){this.root.className=this.root.className+" "+this.constructor.prototype.classes.join(" ").trim()}getStyleSheets(){var styles=this["stylesheets"]||[];return styles.reverse()}setStyleDocuments(){this.loadcss(this.getStyleSheets());this.setStylesheet()}async loadcss(urls){urls=urls.reverse();var stylesheets=window.loaded_stylesheets=window.loaded_stylesheets||
{};for(let path of urls){path=this.onLoadStyle(path);if(path&&!stylesheets[path]){var tagName=/^http/.test(path)?"link":"style";var tag=document.createElement(tagName);tag.setAttribute("type","text/css");tag.setAttribute("rel","stylesheet");tag.setAttribute("href",path);tag.setAttribute("component",this.namespace);stylesheets[path]=tag;if(tagName.toLowerCase()=="style"){var _cssText=await window.imports(path);if(_cssText){_cssText=this.onTransformStyle(_cssText);_cssText&&this.setCssTextAttribute(_cssText,
tag);this.onStylesheetLoaded(tag);this.onAppendStyle(tag)}}else this.onAppendStyle(tag)}}}onLoadStyle(url){return url}setPrototypeInstance(){this.root.setAttribute("namespace",this.namespace);this.prototype=this}resourcepath(url,ns){url=url.replace(/\$\{ns\}/gm,ns.replace(/\./gim,"/"));return Config.ROOTPATH+url}initializeChildComponents(el){el=el||this;var self=this;var nodes=this.querySelectorAll("*");nodes=[].slice.call(nodes);nodes.forEach((n)=>{if(n&&n.nodeType==1){var tag=n.tagName.toLowerCase();
