...
var wait=(ms)=>new Promise((r,j)=>setTimeout(r,ms));window.registered_tags=window.registered_tags||{};if(Config.LOGGING==false)for(var k in console)console[k]=function(){};window.getParameterByName=function(name,url){var match=RegExp("[?&]"+name+"=([^&]*)").exec(url||window.location.href);return match&&decodeURIComponent(match[1].replace(/\+/g," "))};window.toAbsoluteURL=function(url){const a=document.createElement("a");a.setAttribute("href",url);return a.cloneNode(false).href};window.classof=function(ns){return NSRegistry[ns]};
