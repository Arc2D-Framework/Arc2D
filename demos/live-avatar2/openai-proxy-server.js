...
// Realtime voice model/voice defaults (mirrors DemoGeeni's OpenAIProvider).
...
/**
 * Mint a short-lived Realtime client secret. The real OPENAI_API_KEY never leaves
 * the server; the browser gets only the ephemeral secret and does WebRTC directly
 * with OpenAI (POST /v1/realtime/calls). Server VAD = automatic turn-taking + barge-in.
 * Also enables input transcription so the element can surface user transcript events.
 */
async function handleOpenAIRealtimeSessionRequest(request, response) {
    if (!openAIKey) {
    ...
    try {
        const rawBody = await readRequestBody(request);
        const body = rawBody ? JSON.parse(rawBody) : {};
        const model = typeof body.model === 'string' && body.model ? body.model : OPENAI_REALTIME_MODEL;
        const voice = typeof body.voice === 'string' && body.voice ? body.voice : OPENAI_REALTIME_VOICE;
        ...
        const upstream = await fetch('https://api.openai.com/v1/realtime/client_secrets', {
            ...
            body: JSON.stringify({
                session: {
                    ...
                    audio: {
                        input: {
                            // server VAD = automatic turn-taking + barge-in
        ...
        if (!upstream.ok || !raw?.value) {
            sendJson(response, upstream.status || 502, {
                error: raw?.error?.message || 'Failed to mint Realtime client secret.',
                rawResponse: raw
            });
            return;
        }
        sendJson(response, 200, { ok: true, clientSecret: raw.value, model, voice });
    } catch (error) {
...
const server = http.createServer(async (request, response) => {
    ...
    if (request.method === 'POST' && requestUrl.pathname === '/api/openai/realtime-session') {
        await handleOpenAIRealtimeSessionRequest(request, response);
        return;
    }
...
server.listen(port, () => {
    ...
    console.log(`[openai-proxy-server] Live avatar: http://localhost:${port}/demos/live-avatar2/live-avatar.html`);
});
