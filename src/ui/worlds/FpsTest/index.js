import '@ui.worlds.entities.canvas.Planet';
    ...
    class FpsTest extends World {
        ...
        async onConnected() {
			...
			var smallerDimension = Math.min(window.innerWidth, window.innerHeight),
			    ...
			    sun = new ui.worlds.entities.canvas.Planet({x: this.canvas.width*0.5, y: this.canvas.height * 0.5}, sunRadius, 0, 0, '#FFD000'),
			    earth = new ui.worlds.entities.canvas.Planet(sun, earthRadius, earthOrbitRadius, 0.03 * Math.PI / 180, 'blue'),
			    moon = new ui.worlds.entities.canvas.Planet(earth, moonRadius, moonOrbitRadius, 0.1 * Math.PI / 180, 'gray'),
			    moon2 = new ui.worlds.entities.canvas.Planet(moon, moon2Radius, moonOrbitRadius, 0.3 * Math.PI / 180, 'gray');
