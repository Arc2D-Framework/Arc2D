...
class TourGuide extends HTMLElement {
    ...
    async setTarget(step) {
        ...
        if (this.target) {
            ...
            if (step.onadvance && step.onadvance != "valid") {
                this._advanceListener && this._advanceTarget?.removeEventListener(this._advanceEvent, this._advanceListener);
                this._advanceListener = async e => {
                    step.success = true;
                    setTimeout(e => this.next(), step.waitfor || 100);
                };
                this._advanceEvent = step.onadvance;
                this._advanceTarget = this.target;
                this.target.addEventListener(step.onadvance, this._advanceListener, { once: true });
            }
    ...
    // }
    unsetTarget() {
        if (this._advanceListener && this._advanceTarget) {
            this._advanceTarget.removeEventListener(this._advanceEvent, this._advanceListener);
            this._advanceListener = null;
            this._advanceTarget = null;
            this._advanceEvent = null;
        }
