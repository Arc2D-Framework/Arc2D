...
class TourGuide extends HTMLElement {
    ...
    async restore(index) {
        ...
        if (index > 0) {
            ...
            if (index <= allSteps.length) { // ✅ Fix: Allow restoring last step too
                this.current = allSteps[index - 1]; // ✅ Fix: Subtract 1 for one-based indexing
                // this.history = allSteps.slice(0, index); // ✅ Keep history clean
                this.historyIndex = index - 1; // ✅ Fix: Align history index correctly
            } else {
        ...
        // ✅ Fix: directly display current if restoring state
        debugger
        if (this.current) {
            ...
            if (el) {
                ...
                // this.setTitle(this.current.title);
                // await this.setContent(this.current.content);
                // this.setPosition(this.current);
                // this.playSound("show");
            } else {
                console.warn(`'target node' for restored step not found.`);
                this.next(true);
                return;
            }
    
            // this.nextStep = this.history[this.historyIndex + 1] || null; // ✅ Fix nextStep calculation
            this.update();
            setTimeout(() => this.classList.add("active"), 200);
        } else {
            this.next(); // ✅ normal logic if not restoring state
        }
    }
    ...
    async next(skip = false) {
        ...
        // ✅ Handle `onadvance` AFTER state is set
        if (!this.current?.success) {
            if (this.current?.onadvance === "valid" && this.target) {
                if (!this.target.checkValidity()) {
                    this.shake();
                    this.playSound("error");
                    setTimeout(() => {
                        alert(this.target.validationMessage);
                        this.target.focus();
                    }, 100);
                    return;
                } else {
        ...
        if (
            this.current?.onadvance && 
            this.current.onadvance !== "blur" && 
            !this.current?.success && 
            !skip
        ) {
            this.shake();
            this.playSound("error");
            setTimeout(() => this.target?.focus() || console.warn("Target is missing. Cannot focus."), 100);
            return;
        }
        ...
        if (nextStep) {
            if (this.isValidUrl(nextStep) && this.shouldRedirect(nextStep)) {
                if (this.current.interactive && this.current.success) {
                ...
                else if (!this.current.interactive) {
                    sessionStorage.setItem('activeTourKey', this.currentTourKey);
                    sessionStorage.setItem('activeTourStepIndex', this.historyIndex + 1);
                ...
                // ✅ Jump to next step if it's a key
                this.show(nextStep);
                return;
            }
        }

        // ✅ Advance to next step using array-based state
        if (this.historyIndex === -1) {
            ...
            console.warn("No more steps. Ending tour.");
            this.onFinish();
            return;
        }

        //Reset .success after leaving the step
        if (this.current && ("success" in this.current)) {
            this.current.success = false;
        }
        this.unsetTarget();
        // ✅ Set the current step AFTER adjusting index
        this.current = this.history[this.historyIndex];

        // ✅ Explicitly set success state to avoid undefined handling
        if (this.current.success === undefined) {
            this.current.success = false;
        }


        // ✅ Set target and display step FIRST before checking onadvance
        if (this.current) {
            const el = await this.setTarget(this.current);
            if (el) {
                await this.setStep(this.current);
            } else {
                console.warn(`Target node for step not found. Skipping step: "${this.current.title}"`);
    ...
    async previous() {
        ...
        if (this.historyIndex > 0) {
            await this.unsetTarget(); // Clean up previous state
    
            this.historyIndex--;
            this.current = this.history[this.historyIndex];
    
            // ✅ Handle reset of validation state if required
            ...
            if (el) {
                ...
                console.warn(`'Target node' for previous step not found. Skipping back to last known step.`);
                if (this.historyIndex > 0) {
                    await this.previous(); // ✅ Keep going back if the target is missing
                }
            }
            this.update();
        }
    ...
    update() {
        ...
        this.btnSkip.style.display = this.current?.skipLabel === null ? "none" : "inline-flex";
