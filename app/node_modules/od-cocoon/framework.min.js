...
}else{var src=await response.text();console.error(error,src);resolve(null)}}catch(e){var request=new XMLHttpRequest;request.open("GET",path,false);request.send(null);if(request.status==0||request.status==200){src=request.responseText;window.imported_classes[x]=src;resolve(src)}}})};function transpile(target,level){}function stylesheets(target,paths){target.prototype["@stylesheets"]=paths}function css(target,paths){target.prototype["@stylesheets"]=paths}
function html(target,path){target.prototype["@template-uri"]=path}function traits(target,__traits){var inheritTraits=function(klass,properties){properties=properties.reverse();properties.forEach((trait)=>{if(typeof trait=="object")defineProps(klass,trait)})};var defineProps=function(proto,trait){for(var prop in trait)if(!proto[prop])Object.defineProperty(proto,prop,{value:trait[prop],writable:true})};inheritTraits(target.prototype,__traits)}
function cascade(target,shouldCascade){target.prototype["@cascade"]=shouldCascade}function prop(target,key,val){target.prototype[key]=val}function loggable(target,params){}function tag(target,name){window.customElements.define(name,target);return}function field(target,type,key,val){target=type=="static"?target:target.prototype;target[key]=val}
function matchmedia(target,queryStr,templatePath){console.log("matchmedia target",target);var mql=window.matchMedia(queryStr);mql.addListener(()=>{if(mql.matches)target.prototype.onMediaQueryChanged(mql,templatePath)});if(mql.matches)target.prototype.onMediaQueryChanged(mql,templatePath)}
(function(env){env.NSRegistry=env.NSRegistry||{};env.namespace=function(ns,def){def=def||{};var k=def&&typeof def=="object"?def:def.prototype;k.namespace=ns;k.classname=ns.match(/\.?([a-zA-Z0-9_]*)$/)[1];var n=createNS(ns);env.NSRegistry[ns]=n[0][n[1]]=def?createClass(def,ns):{};return env.NSRegistry[ns]};var createNS=function(aNamespace){var scope=env;var parts=aNamespace.split(/\./g);var classname=parts.pop();for(var i=0;i<=parts.length-1;i++)scope=scope[parts[i]]||(scope[parts[i]]={});return[scope,
classname]};var createClass=function(properties,ns){if(typeof properties=="function"){properties.prototype.ancestor=properties.prototype.__proto__.constructor;return properties}}})(window);namespace("core.traits.ResourcePathTransformer");
...
namespace("w3c.ui.WebComponent",class extends HTMLElement{constructor(){super();this.__proto=this.constructor.prototype;this.root=this.onEnableShadow()?this.attachShadow({mode:"open"}):this}setStylesheet(){var css=this.cssStyle();!!css&&!this.__proto._style_defined?(this.appendStyleSheet(`<style type="text/css" rel="stylsheet">\n${css}\n</style>`.toDomElement()),this.__proto._style_defined=true):null}querySelector(cssSel,deep){return deep?this.root.querySelector(cssSel):super.querySelector(cssSel)}querySelectorAll(cssSel,
deep){return deep?this.root.querySelectorAll(cssSel):super.querySelectorAll(cssSel)}appendStyleSheet(stylesheet){if(this.onEnableShadow())this.root.host.appendChild(stylesheet);else{var headNode=application.head;var configscript=application.configscript;headNode.insertBefore(stylesheet,configscript)}}animate(className){this.classList.add("animated",className);setTimeout(()=>{this.classList.remove("animated",className)},1E3)}adopts(orphan){orphan&&orphan.parentNode.replaceChild(this,orphan);orphan&&
this.appendChild(orphan)}replaces(orphan){orphan&&orphan.parentNode.replaceChild(this,orphan)}dispatchEvent(type,data,details={bubbles:true,cancelable:true,composed:true},element=this){var evt=new CustomEvent(type,details);evt.data=data;return super.dispatchEvent(evt)}bind(el,evtName,handler){var self=this;if(typeof el=="string")this.addEventListener(evtName,(e)=>{var t=this.getRealTargetFromEvent(e,el);if(t)handler({target:t,realtarget:e.target,src:e,preventDefault:()=>e.preventDefault,stopPropagation:()=>
e.stopPropagation})},true);else el.addEventListener(evtName,handler,false)}getParentBySelectorUntil(elem,terminator,selector){elem=elem||this;terminator=terminator||"body";var parent_node=null;for(;elem&&elem!==document;elem=elem.parentNode){if(terminator)if(elem.matches&&elem.matches(terminator))break;if(selector)if(elem.matches&&elem.matches(selector)){parent_node=elem;break}}return parent_node}getRealTargetFromEvent(e,selector,terminator){var el=e.composedPath()[0];return this.getParentBySelectorUntil(el,
terminator,selector)}onStylesheetLoaded(style){}cssTransform(css){var ns=this.namespace;css=css.replace(/resource\(['"]?([^'"]*)['"]?\)/mg,(full,m1)=>"url("+this.resourcepath(m1,ns)+")");return css}setCssTextAttribute(_cssText,stylenode){if(stylenode&&stylenode.styleSheet)stylenode.styleSheet.cssText=_cssText;else stylenode.appendChild(document.createTextNode(_cssText))}async loadTemplate(){return new Promise(async(resolve,reject)=>{var tem=this.querySelector("template")||this.src||this.template()||
"/src/./templates/index.html";if(/\/*\.html$/.test(tem)){var src=this.src||tem;var opts={cache:"force-cache"};src=src.replace("/./","/"+this.namespace.replace(/\./gim,"/")+"/");this._template=await imports(src,opts)}else if(/<\s*\btemplate\b/.test(tem))this._template=tem;else if(tem&&tem.nodeType==1)this._template=tem.outerHTML;resolve(this._template)})}onConnected(){}render(data){data=data||{};data.component=this;var t=this._template;if(t){var html=this.evalTemplate(t,data||{});var temNode=html.toDomElement();
temNode=temNode.content;if(!this.onEnableShadow()){this.slots.forEach((slot)=>{var slotName=slot.getAttribute("slot");var placeholder=temNode.querySelector(`slot[name="${slotName}"]`);if(placeholder){placeholder.innerHTML="";placeholder.appendChild(slot)}});this.root.innerHTML=""}this.root.innerHTML="";this.root.appendChild(temNode);this.dispatchEvent("rendered")}}static get observedAttributes(){return["src"]}get src(){return this.getAttribute("src")}set src(val){this.setAttribute("src",val)}onEnableShadow(){return this.getAttribute("attach-shadow")!=
...
this}setClassList(){var classes=[];if(this["@cascade"]==true){var ancestor=this.ancestor;while(ancestor&&ancestor.prototype["@cascade"]==true){var proto=ancestor.prototype;classes.unshift(proto.classname);ancestor=proto.ancestor;if(!ancestor||ancestor==HTMLElement)break}}classes.push(this.classname);var newClsName=this.className+(" "+classes.join(" "));this.className=newClsName.trim()}});transpile(w3c.ui.WebComponent,"es7");traits(w3c.ui.WebComponent,[core.traits.ResourcePathTransformer]);
transpile(w3c.ui.WebComponent,"es7");cascade(w3c.ui.WebComponent,true);namespace("w3c.ui.Application",class extends w3c.ui.WebComponent{constructor(element){super(element);window.application=this;this.head=document.getElementsByTagName("head")[0];this.configscript=document.querySelector("script[id='config']")||document.querySelector("script")}template(){return""}});function Ecmascript6ClassTranspiler(){}Ecmascript6ClassTranspiler.prototype.imports=window.imports;
